<?php
/**
 * Belvilla_jspnrpc_class using CURL
 * 
	...
	public function makeCall($call,$params = array()) { 
		...
		if ($result = curl_exec($ch)) {
			...
			if($res = json_decode($result)){
				if(isset($res->error)){//checken als er geen error teruggegeven is, anders Exception gooien
					$this->__expObj = new Exception($res->error->message,$res->error->code);
					$this->__logError();
					
					throw $this->__expObj;
				}
			}else{//checken als de jsondecode geen fouten geeft, anders Exception gooien met daarin de json_last_error()
				$this->__expObj = new Exception(json_last_error());
				$this->__logError();
				
				throw $this->__expObj;
			}
		}else{ //checken als de curl geen fouten geeft, anders Exception gooien met daarin de curl_error
