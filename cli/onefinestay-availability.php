...
class OneFineStayAvailability extends Import
{
  ...
  public function doExecute()
  {
    ...
    foreach ($properties as $property)
    {
      ...
      $availability = $this->getData('http://partners.onefinestay.com/api/properties/' . $property->id . '/availabilities/', $this->api_key);
      ...
      try
        {
          ...
          foreach ($availability->busy as $avper)
          {

              $availabilityObj = new stdClass;
              // Start date of the availability period
              $availabilityObj->start_date = JFactory::getDate($avper->start_date)->calendar('d-m-Y');

              // Adjust the end date so it's the Friday rather than the following satrday
              $availabilityObj->end_date = JFactory::getDate($avper->end_date)->calendar('d-m-Y');
    ...
    {
      $user_id = JUser::getInstance('SquareBreak')->id;
...
JApplicationCli::getInstance('OneFineStayAvailability')->execute();
