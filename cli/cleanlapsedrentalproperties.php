...
class CleanLapsedRealEstateAffiliateProperties extends JApplicationCli
{
  ...
  var $users_to_clean = array( 11537,11052, 12344, 12430);
    ...
    foreach ($this->users_to_clean as $user_id)
    {
      ...
      foreach ($props as $k => $v)
      {
        ...
        try
        {
          ...
          $query = $db->getQuery(true);
          $query->delete('`#__unit`');
          $query->where(' id = ' . (int) $v->UNITID);
          $this->delete($db, $query);

          // Delete from the property versions table.
          $query = $db->getQuery(true);
          $query->delete('`#__unit_versions`');
          $query->where(' unit_id = ' . (int) $v->UNITID);
          $this->delete($db, $query);

          // Delete from the property table
          $query = $db->getQuery(true);
          $query->delete('`#__unit_attributes`');
          $query->where(' property_id = ' . (int) $v->UNITID);
          $this->delete($db, $query);
          ...
          // Delete from the property views table
          $query = $db->getQuery(true);
          $query->delete('`#__tariffs`');
          $query->where(' unit_id = ' . (int) $v->UNITID);
          $this->delete($db, $query);

          // Delete from the property views table
          $query = $db->getQuery(true);
          $query->delete('`#__availability`');
          $query->where(' unit_id = ' . (int) $v->UNITID);
          $this->delete($db, $query);

          $f = $this->image_path . $v->UNITID;
