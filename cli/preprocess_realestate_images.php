...
class PreProcessRentalImages extends JApplicationCli
{
  ...
  private $users_to_ignore = '8290, 7934, 9773';
  ...
  public function doExecute()
  {
    ...
    foreach ($images as $image)
    {
      ...
      try
      {
        ...
        $image_path_to_copy = 'C:\xampp\htdocs\images\property' . '/' . (int) $image->id . '/' . $image->image_file_name;
        ...
        if (file_exists($image_path) && !file_exists($image_path_to_copy))
        {
          ...
          if (!$move)
          {
            throw new Exception('Problem moving image ' . $image->image_file_name . ' for unit ' . $image->id);
          }
        }
      }
      catch (Exception $e)
      ...
      $this->out('Done image...' . $image->image_file_name . ' for unit ' . $image->id);
    }
  }
  ...
  private function _getImages()
  {
    ...
    $query->select('a.id, b.image_file_name');
    ...
    $query->where('a.created_by not in (' . $db->quote($this->users_to_ignore) . ')');
