...
<?php if (empty($progress['units'])) : // Listing has no units ?>
  <?php $notices[] = JText::_('COM_RENTAL_LISTING_COMPLETE_PLEASE_COMPLETE_ACCOMMODATION_DETAILS'); ?>
<?php elseif (!empty($progress['units'])) : // Listing has unit(s) ?>
  <?php foreach ($progress['units'] as $key => $value) : ?>
    <?php foreach ($value as $section => $complete) : ?>
      <?php if (!$complete) : ?>
        <?php $notices[] = JText::sprintf('COM_RENTAL_HELLOWORLD_LISTING_PROGRESS_NOTICES', $section, $key); ?>
      <?php endif; ?>
    <?php endforeach; ?>
  <?php endforeach; ?>
<?php endif; ?>
...
<?php elseif (empty($notices) && $progress['review']) : ?>
  <div class="well well-small">
    <form action="<?php echo JRoute::_('index.php?option=com_rental&task=propertysubmit&id=' . (int) $progress['listing']); ?>" method="post" name="" id="" class=" ">
