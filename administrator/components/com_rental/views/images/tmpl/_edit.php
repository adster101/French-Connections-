...
<?php if ($format == 'html') : ?>
  <div id="collapseUpload" class="collapse row-fluid">
    <div class="span12">
      <form action="<?php echo JRoute::_('index.php?option=com_rental&task=images.upload&' . JSession::getFormToken() . '=1&id=' . (int) $this->item->id) . '&property_id=' . $this->item->property_id; ?>" method="post" name="imageUpload" id="imageForm">
        <div class="row-fluid">
          <div class="span12">
            Some sensible image manager at last?
            <h2><?php echo JText::_('COM_RENTAL_IMAGES_UPLOAD_IMAGES'); ?></h2>
            <p><?php echo JText::_('COM_RENTAL_IMAGES_UPLOAD_IMAGES_HELP'); ?></p>
            ...
            <div class="clearfix">
              <button class="btn btn-large" id="explore">
                ...
                <?php echo JText::_('COM_RENTAL_IMAGES_BROWSE_IMAGES'); ?>
              </button>      
...
<form action="<?php echo JRoute::_('index.php?option=com_rental&view=images&task=edit&id=' . (int) $this->item->id); ?>" method="post" name="adminForm" id="adminForm" class="form-validate image-manager">
  <div class="row-fluid">
    <?php foreach ($image_properties as $woot => $value) : ?>
      <?php if (array_key_exists('gallery', $image_properties) && array_key_exists('library', $image_properties)) { ?>  
        ...
        <?php } else { ?>
          ...
          <div class="thumbnail">  
            <fieldset class="adminform">
              <legend><?php echo JText::_('COM_RENTAL_IMAGES_IMAGE_' . $woot); ?></legend> 
              <?php if (!count($image_properties[$woot])): ?>
                <ul class="hero-unit" id="<?php echo $woot; ?>">
                  <?php if ($this->item->property_id != 1) { ?>  
                    <li><p class="no-images-in-gallery"><?php echo JText::_('COM_RENTAL_IMAGES_NO_IMAGES_ASSIGNED_TO_GALLERY_' . $woot); ?> </p></li>
                  <?php } else { ?>
                    <li><p class="no-images-in-gallery"><?php echo JText::_('COM_RENTAL_IMAGES_NO_IMAGES_ASSIGNED_TO_' . $woot); ?> </p></li>
                      ...
                      <li>
                        <div class="handle thumbnail">
                          ...
                          <span class="pull-right">
                            <a rel="woot" 
                               title="<?php echo JText::_('COM_RENTAL_HELLOWORLD_IMAGES_DELETE_IMAGE') ?>"
                               href="<?php echo JRoute::_('index.php?option=com_rental&view=deleteimage&format=raw&property_id=' . (int) $this->item->property_id . '&property_id=' . (int) $this->item->id) . '&id=' . (int) $image->id . '&' . JSession::getFormToken() . '=1'; ?>"
                          ...
                          <span class="pull-right">
                            <a rel="woot" 
                               title="<?php echo JText::_('COM_RENTAL_HELLOWORLD_IMAGES_EDIT_CAPTION_IMAGE') ?>"
                               href="<?php echo JRoute::_('index.php?option=com_rental&view=caption&format=raw&property_id=' . (int) $this->item->id) . '&id=' . (int) $image->id . '&' . JSession::getFormToken() . '=1'; ?>" 
                          ...
                          $caption = ($image->caption ? $image->caption : JText::_('COM_RENTAL_HELLOWORLD_IMAGES_NO_CAPTION_SET_FOR_THIS_IMAGE'));
                          ?>
                          <p>
                            <img
                              class="hasTip" 
