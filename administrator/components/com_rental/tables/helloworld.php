 ...
 * Hello Table class
 */
class RentalTableHelloWorld extends JTableNested {
  ...
  protected function _getAssetName() {
    $k = $this->_tbl_key;
    return 'com_rental.message.' . (int) $this->$k;
  }
  ...
  protected function _getAssetParentId() {
    ...
    $assetParent->loadByName('com_rental');
  ...
  protected function savePropertyTranslation($lang = 'en-GB') {
    ...
    $existingTranslations = JTable::getInstance('HelloWorld_translations', 'RentalTable');
  ...
  protected function savePropertyFacilities($POST = array()) {
    ...
    if (count($attributes) > 0) {
      ...
      $attributesTable = JTable::getInstance($type = 'PropertyAttributes', $prefix = 'RentalTable', $config = array());
  ...
  protected function savePropertyTariffs($POST = array()) {
    ...
    if (!$tariffsTable->save($this->id, $tariff_periods)) {
      JApplication::enqueueMessage(JText::_('COM_RENTAL_HELLOWORLD_NEW_UNIT_TO_BE_ADDED'), 'warning');
  ...
  public function saveImageDetails($POST = array()) {
    ...
    $imagesTable = JTable::getInstance($type = 'Images', $prefix = 'RentalTable', $config = array());
    ...
    if (count($subtree) == 1 && $this->isLeaf($this->id) && $parent_id == 1) { // This must be a single unit property...
      ...
      JApplication::setUserState('com_rental.images.progress', true);
    } else {
      // Set the $parent_id to the property ID so images are assigned to the gallery.

      $image_gallery_table = JTable::getInstance('Gallery_images', 'RentalTable');
