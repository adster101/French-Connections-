 ...
 */
class RentalControllerHelloWorld extends JControllerForm {
    ...
    if ($user->authorise('helloworld.edit.property.owner', 'com_rental')) {
      ...
      if (!empty($data)) {
        ...
        if (array_key_exists('created_by', $data) && $data['created_by'] != 0) {
          ...
          $this->setRedirect(JRoute::_('index.php?option=com_rental&task=helloworld.edit', false));

          return false;
        }

        $app->enqueueMessage(JText::_('COM_RENTAL_HELLOWORLD_PLEASE_CHOOSE_AN_OWNER'), 'error');
      }
      ...
      if (!empty($data)) {
        ...
        if (array_key_exists('parent_id', $data) && $data['parent_id'] != 0) {
          ...
          $this->setRedirect(JRoute::_('index.php?option=com_rental&task=helloworld.edit', false));
          if ($data['parent_id'] == 1) {
            $app->enqueueMessage(JText::_('COM_RENTAL_HELLOWORLD_NEW_PROPERTY_TO_BE_ADDED'), 'notice');
          } else {
            $app->enqueueMessage(JText::_('COM_RENTAL_HELLOWORLD_NEW_UNIT_TO_BE_ADDED'), 'notice');
          }
