...
class HelloWorldTableImages extends JTable
{
  ...
  public function save ($id = null, $images = array(), $parent_id = 1, $map_array = false ) 
  {
    ...
    if (!$this->check($images)) {
      ...
      $query->insert('#__property_images');
      
			$query->columns(array('property_id','parent_property_id','image_url','image_file_name','caption'));
      
      foreach ($images as $image) {
        // Only insert if there are some images 
        if ($image['image_url'] !='') {
          $insert_string = "$id, $parent_id,'" . $image['image_url'] . "','" . $image['image_file_name'] . "','". $image['caption'] . "'";
