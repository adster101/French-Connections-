			...
			if (count > 1) {
				...
				{						
					if (ui.item.position().top != ui.originalPosition.top){
						$('[type=text].text-area-order', ui.item).attr('value', parseInt($('[type=text].text-area-order', ui.item.next()).attr('value')));
					}
					$(range).each(function () {
						...
						if ( ui.item.get(0) !== $(this).get(0)){	
							if (_top > ui.item.position().top && _top <= ui.originalPosition.top) {
								if (sortDir == 'asc') {
									var newValue = parseInt($('[type=text].text-area-order', $(this)).attr('value')) + 1;
								} else {
									var newValue = parseInt($('[type=text].text-area-order', $(this)).attr('value')) - 1;
								}
	
								$('[type=text].text-area-order', $(this)).attr('value', newValue);
							}
						}
					});
				} else if (ui.originalPosition.top < ui.position.top) {
					if (ui.item.position().top != ui.originalPosition.top){
						$('[type=text].text-area-order', ui.item).attr('value', parseInt($('[type=text].text-area-order', ui.item.prev()).attr('value')));
					}					
					$(range).each(function () {												
						var _top = $(this).position().top;
						if ( ui.item.get(0) !== $(this).get(0)){						
