...
class FcSearchModelSearch extends JModelList
{
    ...
    public function getLocalInfo()
    {
        ...
        $query->select(
                ...
                $db->quoteName('metadescription') . ',' .
                $db->quoteName('metakeywords')
        );
        if ($lang == 'fr')
        {
    ...
    protected function getListQuery()
    {
        ...
        try
        {
            ...
            // If the search level is for a town or city then add a distance field
    ...
    {
        $query->where('(select title from qitz3_special_offers k left join qitz3_property kk on kk.id = k.property_id where k.published = 1 AND k.start_date <= ' . $db->quote($this->date) . 'AND k.end_date >= ' . $db->quote($this->date) . ' and kk.created_by != 11052 and k.unit_id = d.unit_id limit 0,1) is not null');
    ...
    private function getFilterPrice(JDatabaseQueryMysqli $query, $min_price, $max_price)
    {
