<?php
/**
 * @version: $Id: entry.php 2294 2012-03-12 12:15:27Z Radek Suski $
...
class SPEntryCtrl extends SPController
{
	...
	protected function submit()
	{
		...
		if( !( SPRequest::int( 'entry_parent', 0  ) ) ) {
			$error = Sobi::Txt( 'You have to select at least one category' );
		...
		if( !( SPFactory::cache()->addVar( array( 'post' => $_POST, 'files' => $_FILES, 'store' => $store ), 'request_cache_'.$tsid ) ) ) {
...
//		}
		...
		if( $pCount && !( Sobi::Can( 'entry.payment.free' ) )  ) {
			...
			if( !( SPFactory::cache()->addObj( $data, 'payment', $sid, 0, true ) ) ) {
