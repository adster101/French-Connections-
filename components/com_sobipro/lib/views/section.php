<?php
/**
 * @version: $Id: section.php 2309 2012-03-20 18:38:08Z Radek Suski $
 ...
 * ===================================================
 * $Date: 2012-03-20 19:38:08 +0100 (Tue, 20 Mar 2012) $
 * $Revision: 2309 $
 * $Author: Radek Suski $
...
class SPSectionView extends SPFrontView implements SPView
{
    ...
    protected function cachedEntry( $entry, $manager, $noId = false )
    {
        ...
        if ( is_array( $en ) && count( $en ) ) {
            if ( strstr( SPRequest::task(), 'search' ) || $noId ) {
    ...
    protected function entry( $entry, $manager, $noId = false )
    {
        ...
        if ( !( is_array( $en ) ) || !( count( $en ) ) ) {
            ...
            SPFactory::cache()->addObj( $entry, 'entry', $entry->get( 'id' ) );
            SPFactory::cache()->addObj( $en, 'entry_struct', $entry->get( 'id' ) );
            unset( $en[ 'url_array' ] );
            unset( $en[ 'edit_url_array' ] );
            unset( $entry );
        }
